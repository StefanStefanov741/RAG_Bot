...
files_dir = "D:\RAG Research\HTML_files"
...
if(not db_exists):
    initial_elements = html_to_elements(file_path=input_files_list[0])
    ...
    if not db is None:
        ...
        for file_path in input_files_list:
            if os.path.isfile(file_path) and (not skip_file==file_path):
                new_pdf_elements = html_to_elements(file_path=file_path)
        ...
        bot_answer = my_bot.ask("What is the best method of brewing coffee fast?",db)
